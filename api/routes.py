...
async def generate_report(request: ReportRequest):
    ...
    try:
        # Always use the provider configured in .env/settings.
        # This prevents the frontend from accidentally overriding
        # Ollama with Gemini.
        orchestrator = OrchestratorAgent(
            llm_provider=settings.llm_provider
        )
...
async def get_trends():
    ...
    try:
        ...
        trends = await run_in_threadpool(
            td.detect_trending_topics
        )

        timeline = await run_in_threadpool(
            td.get_publication_timeline
        )

        top_papers = await run_in_threadpool(
            td.get_most_referenced_papers
        )
